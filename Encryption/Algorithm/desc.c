...
static const UINT2 BYTE_BIT[8] = {
  0200, 0100, 040, 020, 010, 04, 02, 01
};

static const UINT4 BIG_BYTE[24] = {
...
static const unsigned char TOTAL_ROTATIONS[16] = {
		...
		unsigned int len)
{
	...
	DESKey(subkeys[0], encrypt ? key : &key[16], encrypt);
	DESKey(subkeys[1], &key[8], !encrypt);
	DESKey(subkeys[2], encrypt ? &key[16] : key, encrypt);
	...
	for(i = 0; i < len/8; i++) {
		...
		*(work+1) = *(inputBlock+1);
		...
		DESFunction(work,subkeys[0]);
		DESFunction(work,subkeys[1]);
		DESFunction(work,subkeys[2]);

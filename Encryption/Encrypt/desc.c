		...
		unsigned int len)
{
	...
	for(i = 0; i < len/8; i++) {
		...
		*(work+1) = *(inputBlock+1);
		...
		desfunc(work,subkeys[0]);
		desfunc(work,subkeys[1]);
		desfunc(work,subkeys[2]);
...
int maindes()
{
	...
	unsigned char output1[200];
	unsigned char input[200];
	unsigned int len;

	int encrypt;

	memcpy( key, "i love u", 8 );
	encrypt=1;
	len=120;
	memcpy(input,"ScanDisk checked drive C for problems, with the following results: Directory structure Scan Disk did not perform any.\0",len+1);
	DES3(encrypt,key,output,input,len);

	encrypt=0;
	DES3(encrypt,key,output1,output,len);
